<?php
    ...
    group('deploy',function() {
        ...
        require( './wp/wp-blog-header.php' );\" > {$app->env->release_dir}/index.php"
            ...
            if($app->env->releases === false)
            {
                ...
                $cmd[] = "mkdir -p {$app->env->release_dir}/public/themes";
                $cmd[] = "mkdir -p {$app->env->release_dir}/public/plugins";
            }else
            {
                $cmd[] = "mkdir -p {$app->env->shared_dir}/uploads";
                $cmd[] = "mkdir -p {$app->env->release_dir}/public";
                $cmd[] = "rm -rf {$app->env->release_dir}/public/themes";
                $cmd[] = "rm -rf {$app->env->release_dir}/public/plugins";
    ...
    task('compatibility_mode', function($app) {
        ...
        $cmd = array(
            ...
            "ln -s {$app->env->release_dir}/public/plugins {$app->env->release_dir}/wordpress/wp-content/plugins",
